...
is solving many of the same issues. Here's why I didn't use Viper:
...
- "Cmd" suffix is going to conflict with linters that expect the comment
  to match the function name exactly.
  Maybe could use file suffix instead, which would be easily filtered by linters?
